...
def _step_config(config: dict, repo_root: str, bin_dir: str,
    ...
    # COMMENT: prefer the user's ``description`` (it's meant for humans
    # reading the modset) and fall back to an auto-generated string for
    # projects that don't set one. A short, project-specific comment is
    # what we expect — the shipped ``modset_river_30h.json`` carries
    # "30-hour multi-pulse flood simulation with sediment transport" in
    # this slot, and the template's ``description`` default matches.
    modset_comment = (config.get("modset_comment")
                      or config.get("description")
                      or f"Generated by model_config_template.py "
                         f"({config['project_name']})")

    # ------------------------------------------------------------------
    # Compute every optional section FIRST, then assemble the modset in
    # the canonical top-level key order. This order matches the shipped
    # ``modset_river_30h.json`` reference byte-for-byte:
    #
    #   COMMENT, DEM_FILE, SIM_DATETIME_START, RESTART,
    #   [MESH_TYPE, MESH_FILE, MESH_FORMAT, BOUNDARY_CONDITIONS],
    #   [METEO_FILE],
    #   [INFLOW_FILE],
    #   OUTPUT, ROUGNESS_HEIGHT,
    #   [STEADY_STATE],
    #   [SOIL_INFILTRATION],
    #   EXTERNAL_MODULES
    # ------------------------------------------------------------------

    # --- Triangular mesh block (optional) ---
    trimesh_block = {}
    if config["mesh_type"] == "triangular" and mesh_meta is not None:
        ...
        trimesh_block = {
            ...
            "MESH_FILE": _rel(mesh_meta["output_path"], repo_root),
            "MESH_FORMAT": "gmsh",
            "BOUNDARY_CONDITIONS": {"1": {"type": bc_default}},
        }

    # --- Inflow block (optional; accepts either
    #   (1) a plain string path (legacy — DISCHARGE_LOCATION defaults to (0, 0))
    #   (2) a dict with {path, lon, lat}           ★ recommended — WGS84
    #   (3) a dict with {path, x_utm, y_utm}       escape hatch)
    ...
    if inflow:
        inflow_path = None
        x_utm = y_utm = 0
        reprojected = False  # tracks whether the coord came from a
                             # pyproj reprojection (so we don't
                             # artificially coerce to int)
        if isinstance(inflow, dict):
            ...
            if "lon" in inflow and "lat" in inflow:
                ...
                x_utm = inflow.get("x_utm", 0)
                y_utm = inflow.get("y_utm", 0)
        else:  # str
            inflow_path = inflow
            x_utm = config.get("inflow_x_utm", 0)
            y_utm = config.get("inflow_y_utm", 0)

        # Serialisation rule: keep ints as ints (matches hand-written
        # references); keep reprojected floats as floats (sub-metre
        # precision matters there).
        def _coord(v):
            if isinstance(v, bool):  # bool is a subclass of int; guard it
                return int(v)
            if isinstance(v, int) and not reprojected:
                return v
            return float(v)
        ...
        if inflow_path:
            inflow_block = {
                "INFLOW_FILE": {
                    ...
                    "DISCHARGE_LOCATION": {
                        "X_COORDINATE": _coord(x_utm),
                        "Y_COORDINATE": _coord(y_utm),
                    },
                }
            }

    # --- Steady-state section (optional) ---
    steady_block = {}
    ss = config.get("steady_state") or {}
    if ss:
        steady_block["STEADY_STATE"] = {
            "STATUS": bool(ss.get("enabled", False)),
            "TOLERANCE": float(ss.get("tolerance", 0.001)),
            "MIN_TIME": int(ss.get("min_time_s", 1800)),
        }

    # --- Soil-infiltration section (optional; FLUXOS C++ schema:
    #     SOIL_MAP + SOIL_TYPES{id → USDA_CLASS}, not the old
    #     DEFAULT_KS_MM_HR that read_soil_config() never consumed).
    # Template-facing config:
    #   soil_infiltration = dict(
    #       enabled    = True,
    #       soil_map   = "",                    # or "Working_example/soils.asc"
    #       soil_types = { 1: "sandy_loam" },   # id → USDA class (or a dict
    #                                           # with NAME/USDA_CLASS/Ks/…)
    #   )
    # ---
    soil_block = {}
    soil = config.get("soil_infiltration") or {}
    if soil.get("enabled"):
        raw_types = soil.get("soil_types")
        # Backwards-compat: legacy ``default_ks_mm_hr`` maps to a single
        # synthetic type-1 entry so old configs keep working.
        if raw_types is None and "default_ks_mm_hr" in soil:
            raw_types = {1: {
                "NAME": "Uniform",
                "Ks": float(soil["default_ks_mm_hr"]) / 3_600_000.0,  # mm/h→m/s
            }}
        raw_types = raw_types or {1: "sandy_loam"}

        soil_types_out = {}
        for sid, val in raw_types.items():
            if isinstance(val, str):
                usda = val.strip().lower()
                entry = {"NAME": usda.replace("_", " ").title(),
                         "USDA_CLASS": usda}
            elif isinstance(val, dict):
                entry = {}
                if "NAME" in val or "name" in val:
                    entry["NAME"] = val.get("NAME") or val.get("name")
                usda = val.get("USDA_CLASS") or val.get("usda_class")
                if usda:
                    entry["USDA_CLASS"] = str(usda).strip().lower()
                for k_in, k_out in (("Ks", "Ks"), ("f0", "f0"), ("k", "k")):
                    if k_in in val or k_in.lower() in val:
                        entry[k_out] = float(val.get(k_in,
                                                     val.get(k_in.lower())))
            else:
                raise ValueError(
                    f"soil_infiltration.soil_types[{sid!r}]: expected a "
                    f"USDA class string or dict, got {type(val).__name__}")
            soil_types_out[str(sid)] = entry

        soil_block["SOIL_INFILTRATION"] = {
            "STATUS": True,
            "SOIL_MAP": soil.get("soil_map", ""),
            "SOIL_TYPES": soil_types_out,
        }

    # --- Assemble in canonical order -------------------------------------
    modset = {
        "COMMENT": modset_comment,
        "DEM_FILE": _rel(dem_meta["output_path"], repo_root),
        "SIM_DATETIME_START": config["sim_datetime_start"],
        "RESTART": False,
    }
    modset.update(trimesh_block)
    if config.get("meteo_file"):
        modset["METEO_FILE"] = config["meteo_file"]
    modset.update(inflow_block)
    modset["OUTPUT"] = {
    ...
    modset["ROUGNESS_HEIGHT"] = float(config["roughness_height"])
    modset.update(steady_block)
    modset.update(soil_block)
    modset["EXTERNAL_MODULES"] = {
        "ADE_TRANSPORT": {
    ...
    with open(out_path, "w") as f:
        ...
        f.write("\n")  # POSIX-convention trailing newline
