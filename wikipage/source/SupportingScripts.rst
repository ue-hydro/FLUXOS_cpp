...
3D Flood Viewer (``fluxos_viewer.py``)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Interactive 3D visualization of simulation results on top of the terrain DEM. The viewer supports both regular and triangular mesh outputs.
...
* `Google Earth <https://earth.google.com/>`_ -- Overlay inundation maps using KML files generated by ``googleearth_klmgen.py``.
